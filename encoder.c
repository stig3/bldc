...
static const SPIConfig mt6816_spi_cfg = {
        ...
        SPI_SW_CS_GPIO,
        SPI_SW_CS_PIN,
        SPI_BaudRatePrescaler_8 | SPI_CR1_CPOL | SPI_CR1_CPHA | SPI_DATASIZE_16BIT};
#endif
...
void encoder_init_mt6816_spi(void) {
    ...
    palSetPadMode(SPI_SW_SCK_GPIO, SPI_SW_SCK_PIN, PAL_MODE_ALTERNATE(GPIO_AF_SPI1) | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SPI_SW_MISO_GPIO, SPI_SW_MISO_PIN, PAL_MODE_ALTERNATE(GPIO_AF_SPI1) | PAL_STM32_OSPEED_HIGHEST);
    palSetPadMode(SPI_SW_CS_GPIO, SPI_SW_CS_PIN, PAL_MODE_OUTPUT_PUSHPULL | PAL_STM32_OSPEED_HIGHEST);

#if (MT6816_USE_HW_SPI_PINS)
    palSetPadMode(SPI_SW_MOSI_GPIO, SPI_SW_MOSI_PIN, PAL_MODE_ALTERNATE(GPIO_AF_SPI1) | PAL_STM32_OSPEED_HIGHEST);
#endif
...
// if MOSI is defined, use diagnostics
    ...
    if (mode == ENCODER_MODE_MT6816_SPI) {
        ...
        pos = ((reg_data_03 & 0x00FF) << 8) | reg_data_04;
        spi_val = pos;
